...
server programs' unreplicated executions, \xxx merely incurred an average 
througput degrade by \tputoverhead (note that in Figure~\ref{fig:tput}, the 
Y-axises of most programs start from a large number).

As the number of threads increases, \xxx still scales almost as well as the 
unreplicated executions. \memcached itself does not scale well with the number 
of threads, and a prior evaluation~\cite{rex:eurosys14} also observed a similar 
\memcached low scalability.
