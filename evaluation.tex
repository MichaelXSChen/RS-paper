...
\vspace{-.1in}
\caption{{\em Consensus latency breakdown of \xxx and \dare.} The number of 
replicas is seven.}
\label{fig:compare}
\vspace{-.25in}
\end{figure}
...
key and data length. \xxx ran a widely used one, \redis; \dare supported a 
335-line, RDMA-based key-value server written by their authors. To thoroughly 
analyze their latency with scalability, we ran up to seven replicas and 24 
clients. 
...
Table~\ref{tab:compare} shows the consensus latency of \xxx and 
\dare with 50\% SETs and half 50\% requests. \xxx and \dare 
