...
Figure~\ref{tab:compare} shows the consensus latency of \xxx and 
\dare on seven replicas with randomly arriving, 50\% SETs and half 50\% 
requests. When there was only one client, \xxx was slightly slower than \dare 
becuase \dare does not store inputs persistently. On 6 to 24 requests, \xxx was 
\fasterDARElow to \fasterDAREhigh faster than \dare due to two reasons. First, 
\xxx is a one-round protocol and \dare is a two-round protocol, so \dare's 
actual consensus time was \fasterDAREconsensusonly higher than \xxx.
...
the randomly arrival of requests often make the batches small, causing a large 
wait duration (the following batches must wait for prior batches to reach 
consensus and then they invoke consensus). \dare's 
evaluation~\cite{dare:hpdc15} also confirmed a high wait duration: with three 
replicas and nine concurrent clients, \dare's throughput on its real-world 
inspred workloads (50\% SET and 50\% GET arrive randomly) was about 43.5\% lower 
than 100\% SET workloads.

These results suggest that \dare is better on smaller number of 
concurrent requests and replicas (\eg, maintaining leadership 
or metada~\cite{chubby:osdi,zookeeper}), and \xxx is better on larger number of 
concurrent requests or replicas (\eg, replicating server 
