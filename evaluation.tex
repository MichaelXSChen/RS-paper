...
\hline\\[-2.3ex]
\dare & 8.8 & 12.0  & 32.5 & 64.0 & 102.5 \\

% \hline\\[-2.3ex]
% \dare (100W) & 5.8 & 7.2 & 16.8 & 29.2 & 45.1 \\

\end{tabular}
\vspace{-.05in}
\caption{{\em Consensus latency of \xxx and \dare.}}
\label{tab:consensus-latency}
\end{table}

We compared \xxx and \dare~\cite{dare:hpdc15} with the same key-value store 
workload, which contains half GETs and half SETs. This workload represents 
real-world applications such as an advertisement log that records recent user 
activities~\cite{dare:hpdc15}. To thoroughly analyze their latency with 
...
didn't find network a bottleneck when running more replicas on a machine; from 
9 to 105 replicas for both protocols, each RDMA round-trip increased merely 
from 2.6 \us to 4.4 \us.
...
To stress \xxx, we used a replica group size of 9 to run all server 
programs. We spawned up to 32 concurrent connections, and then we measured both 
response time and throughput. We also measured \xxx's bare consensus latency. 
Each performance data point in the evaluation is taken from the mean value of 
...
% number of data bytes received in socket calls (\eg, \mongodb has the largest 
% received bytes).
