...
we ran four \paxos-like protocols on 40Gbps network with only one client 
sending requests. We found that, when changing the replica group size from 
three to nine, the consensus latency of three protocols increased by 
\tradlatencyincreaselow to \tradlatencyincreasehigh, and \systemcostlow to 
\systemcosthigh of this increase was in OS kernel. Therefore, existing 
...
replica's memory without involving the remote OS kernel or CPU. One-sided 
operations is about 2x faster than two-sided operations because a two-sided 
operation contain two one-sided operations~\cite{pilaf:usenix14}. \xxx 
uses one-sided operations, and the rest of this paper denotes such a operation 
as ``WRITE".

RDMA communications between a local NIC and a remote NIC 
...
WRITEs on RC and UC OPs incur almost the same latency, so \xxx 
uses RC QPs.
...
empty, it took 0.039$\sim$0.12 \us; when the CQ has one or more ACKs, it took 
0.051$\sim$0.42 \us. As the number of ACKs is linear to replica group size, 
polling ACKs is a major scalability bottleneck (\S\ref{sec:evaluation}).
