...
high throughput, and its decreasing prices. The speed of RDMA not only comes 
from its kernel bypassing feature, but also its dedicated network stack 
implemented in hardware. Therefore, RDMA is several times faster than 
software-only kernel bypassing techniques (\eg, DPDK~\cite{dpdk}).
...
already succeed before this ACK-checking starts). To achieve scalability, 
\xxx's protocol uses selective signaling and eliminates ACK checkings on every 
consensus round.
