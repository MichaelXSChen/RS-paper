...
called \emph{signaling}). Polling ACK is time consuming. We collected the time 
taken in polling ACKs in DARE~\cite{dare:hpdc15}, a recent RDMA-based \paxos 
protocol, and we found that: if the CQ was empty, it took 61 to 79 \us; if the 
CQ has one or more ACKs, it took 260 to 410 \us. If the number of polling ACK 
operations is linear to the replica group size, a \paxos protocol will incur 
scalability bottlenecks when the size is large (\S\ref{sec:evaluation}).
