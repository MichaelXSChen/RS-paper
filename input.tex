...
\paxos consensus log (\S\ref{sec:overview}). Second, a server 
program thread on the leader replica (in short, a leader thread) which invokes 
a consensus request. For efficiency, \xxx lets leader threads concurrently 
invoke consensus requests when they call inbound socket calls (\eg, \recv). 
Third, a backup eplica's \xxx internal follower thread (\S\ref{sec:overview}) 
which agrees on or rejects consensus requests.
...
Then, each leader thread invokes a consensus on its entry concurrently with 
other leader threads via the third step (\textbf{L3}): WRITE the log entry to 
remote backups. Unlike a previous RDMA-based consensus 
algorithm~\cite{dare:hpdc15} which has to wait for ACKs from remote NICs, our 
WRITE immediately returns after copying the entry to its local QP between the 
leader and each backup. In our evaluation, copying a log entry to local QP took 
