...
the same QP. It addresses this issue with a simple approach. In normal case, the 
...
element for each replica. Normal case operations and heartbeats circularly use 
the other parts of the consensus log. Once backups have not received heartbeats 
from the leader for a period of 3*T, they start to elect a new leader and let 
their follower threads poll from the \v{elect} array.
