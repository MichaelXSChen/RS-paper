...
logging storage on SSD (\S\ref{sec:logging}). Then, \xxx invokes consensus for 
each incoming request (entry) concurrently.
...
packet losses) for our WRITEs. In our evaluation, copying a log entry to local 
QP took no more than 0.2 \us. Therefore, the WRITEs to all backups are done in 
