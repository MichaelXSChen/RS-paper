...
% 
To minimize protocol-level bugs, \xxx's protocol derives from a popular 
viewstamp-based manner~\cite{paxos:practical}. We made this design 
choice because \paxos is notoriously difficult to 
understand~\cite{raft:usenix14,paxos:simple,paxos,paxos:complex} or 
implement~\cite{paxos:live,paxos:practical} 
verify~\cite{modist:nsdi09,demeter:sosp11}. Deriving from a popular 
manner~\cite{paxos:practical} helps us incorporate these 
...
this protocol does not rely on a lossless RDMA. \xxx still provides same 
fault-tolerance guarantee as the standard \paxos model, where a stable storage 
exists, but the hardware or network devices of replicas may fail, and server 
programs may crash.
