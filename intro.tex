...
State machine replication (SMR) runs a program on computer replicas and 
enforces same inputs for this program as long as a quorum 
...
(typically, \paxos~\cite{paxos:practical,paxos,paxos:complex}). 
Recent SMR 
...
protocols, which run on TCP/IP, incur high latency as their messages go 
through TCP/UDP stacks and OS kernels. This latency is acceptable for 
...
six protocols ran 24 concurrent proposing clients. The Y axis is broken 
to fit in all protocols.}
