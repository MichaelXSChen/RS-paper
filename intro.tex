...
% scalability issues.
However, due to the unrichness of RDMA communication primitives, it's 
technically challenging to build a \paxos runtime system that fully exploits 
RDMA's power. For instance, one-sided RDMA operations eliminate remote 
replicas' participations, a counterintuition to traditional \paxos 
protocols in which non-leader replicas must examine the leader's consensus 
requests for an (dis)agreement. To overcome this counterintuition, 
DARE~\cite{dare:hpdc15}, a recent RDMA-based \paxos protocol, conducts a 
sole-leader, two-rounded protocol. First, the leader first does RDMA writes the 
consensus requests to all replicas, polls RDMA ACKs to check whether the writes 
succeed. Second, on a majority of successful writes, the leader then do another 
round of writes to notify other replicas. Our evaluation shows that both polling 
RDMA ACKs and the two-rounded consensus incur scalability bottlenecks on the 
leader side: DARE's consensus latency increased approximately linearly by 
...
Our key idea is that we should make \paxos replicas, including the leader, 
receive consensus messages \emph{purely on their local memory}, totally getting 
rid of socket primitives or RDMA ACK or queue primitives. This is just like 
making threads communicate with other threads efficiently via bare memory.
...
We have implemented this idea in \xxx,\footnote{We name our system after 
falcon, one of the 
