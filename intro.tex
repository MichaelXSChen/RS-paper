...
consistent order of requests for this program as long as a quorum (typically, 
majority) of replicas still behave normally.

Due to this strong consistency and fault-tolerance, \paxos is widely served in 
numerous systems. For instance, a DHT system Scatter~\cite{scatter:sosp11} 
assigns distinct key ranges to \paxos groups and runs 8 to 12 replicas 
in each group to agree on requests for each key range. Moreover, recent state 
machine replication systems~\cite{ crane:sosp15, eve:osdi12, rex:eurosys14} use 
\paxos to greatly improve the availability of general server programs.
