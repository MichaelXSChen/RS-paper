...
We compared \xxx with five popular \paxos-like implementations: 
\libpaxos~\cite{libpaxos}, \zookeeper~\cite{zookeeper}, 
\crane~\cite{crane:sosp15}, \spaxos~\cite{spaxos}, and 
\dare~\cite{dare:hpdc15}. We also evaluated \xxx on \nprog widely used or 
studied server programs, including \nkvprog key-value stores 
...
server \clamav~\cite{clamav}, one multimedia storage server 
