...
Because the second step in this algorithm is parallel, the consensus latency is 
...
inputs is not enough. An automated, efficient output checking mechanism that 
can improve the assurance on ``replicas run in sync" is still missing in 
existing SMR 
systems~\cite{calvin:sigmod12,rex:eurosys14,crane:sosp15,dare:hpdc15}. Nowadays 
most server programs already use multithreading to harness the power of 
multi-core hardware. 
...
approaches either use only \v{ping} to check the liveness of replicas (\eg, 
\redis) or rely on manually annotating share states in program code to detect 
divergence~\cite{eve:osdi12}.
