...
\subsection{Comparing \xxx with \dare}\label{sec:compare}
...
a consensus is reached, leader does WRITEs to notify backups.}
\label{fig:dare}
\vspace{-.20in}
\end{figure}

We highly appreciate \dare~\cite{dare:hpdc15}, a recent fastest \paxos 
protocol (Figure~\ref{fig:dare}) based on RDMA. It is most relevant to \xxx. 
\dare is designed to run with a small number of replicas (\ie, three) and a 
single-threaded key-value server program written by the authors. Under this 
design choice, \dare follows a sole-leader manner that needs to do two-rounds: 
...
to the latest request (tail of consensus log). Both rounds are essential in 
\dare because if a backup becomes a new leader, it needs the tail to perform a 
``log adjustment"~\cite{dare:hpdc15} across replicas.
...
\dare is not designed to scale to a large number of concurrent requests or 
replicas for two reasons. First, its second round needs to update a global 
variable for each replica, which prevents concurrent consensus 
requests, a desirable feature for \paxos performance~\cite{paxos:practical}. 
Therefore, the consensus of a new incoming request in \dare's server program 
must wait for prior all consensus requests to finish (confirmed in our 
evaluation). Second, a sole-leader protocol is not suitable to incorporate a 
stable storage or checkpoint/restore design. Therefore, \dare lacks durability 
(\S\ref{sec:background}), an important feature in standard \paxos protocols 
(including \xxx).
...
Second, \xxx scales well on many concurrent client connections and 
up to nine replicas (\S\ref{sec:eval-dare}), and \dare~\cite{dare:hpdc15} 
mentioned that their design choices do not include scalability. Third, although 
\xxx is a durable protocol and \dare is a volatile one, \xxx is faster than 
\dare by up to \fasterDARE. \S\ref{sec:eval-dare} compares \xxx and \dare 
in detail.
