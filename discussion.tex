...
client connection and they don't have such false positives. A server program 
may have false negative when it triggers a software bug but the bug does not 
propogate to network outputs. On client programs' point of view, such bugs do 
not matter; \xxx already checkpoints file system state to mitigate this issue.

Once execution divergence is detected in a replica, \xxx's rollback mechanism 
is not designed to guarantee that the re-executions of this replica will 
definitely eliminates this divergence. The reason is that both our 
evaluation and a previous work Eve~\cite{eve:osdi12} found that divergence 
happens extremely rarely in evaluation. Specifically, although Eve provided a 
sequential re-execution approach to overcome divergence, which \xxx can 
leverage, but even the evaluation in Eve didn't experience any divergence and 
thus their approach was not in use at all.
