...
only one RDMA round (Figure~\ref{fig:consensus}) and allows concurrent 
consensus requests, while \dare has two rounds. Second, \xxx has shown to scale 
well on many concurrent client connections and up to nine replicas 
(\S\ref{sec:eval-dare}), while \dare~\cite{dare:hpdc15} mentioned that their 
