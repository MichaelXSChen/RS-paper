...
have been conducted to improve specific aspects of consensus, 
including order commutativity~\cite{epaxos:sosp13,marandi:icdcs14}, 
understandability~\cite{raft:usenix14,paxos}, and verifiable 
reliability~\cite{modist:nsdi09,demeter:sosp11}. Recent 
systems~\cite{brun:icdcs12,meling:icdcs12,bal:icdcs12} extend \paxos to tolerate 
byzantine faults~\cite{pbft:osdi99,zyzzyva:sosp07}.
...
Rex~\cite{rex:eurosys14}, and \crane~\cite{crane:sosp15}, use TCP/IP-based 
\paxos protocols to improve the availability of server programs. 
These systems show that SMR services incur modest overhead on 
server programs' throughput compared to unreplicated executions. None of 
these three systems has evaluated their response time overhead on key-value 
servers which are extremely sensitive on latency. \xxx is the first SMR system 
that achieves low overhead on both response time and throughput on real-world 
key-value servers.
...
modules go through TCP/IP layers and incur high consensus latency, and 
\xxx can help them.
