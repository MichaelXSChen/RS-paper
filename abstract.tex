...
protocols can hardly go up to a dozen, because traditional consensus messages 
go throught TCP/IP, thus the consensus latency will increase almost 
linearly to the group size. This paper presents \xxx, a new RDMA-based \paxos 
protocol and its runtime system. \xxx achieves scalability by: (1) carefully 
dividing RDMA workloads among replicas; and (2) making all replicas 
receive consensus messages purely on local memory, just like making threads 
receive other threads' data efficiently on bare memory.

\xxx is the first to achieve low \paxos consensus latency on over 100 replicas. 
Evaluation shows that \xxx's consensus latency was faster than four 
traditional \paxos protocols by \comptradlow to \comptradhigh. When increasing 
the replica group size from three to 105, \xxx's consensus latency increases 
merely from \xxxlatencythree to \xxxlatencyonezerofive, a sub-linear increase. 
\xxx is faster than a recent RDMA-based \paxos protocol by up to \fasterDARE. 
\xxx runs with \nprog widely used, unmodified server programs (\eg, \redis 
and \mysql) with low overhead. All our source code, benchmarks, and raw 
